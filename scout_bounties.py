...
def is_clean_candidate(item, current_repo=None):
    ...
    if comments_val is not None:
        try:
            if int(comments_val) > MAX_COMMENTS:
                return False
    ...
    html_url_lower = html_url.lower()

    # 5. Skip recursive alert issues and self/BountyScout repositories
    if "bounty alert" in title_lower or "🎯" in title:
        return False
    if "active bounty scan results" in body_lower or "/bountyscout/" in html_url_lower:
        return False
    if current_repo and current_repo.lower() in html_url_lower:
        return False

    # 6. Skip cryptocurrency/article writing/spam keywords
    if any(term in title_lower or term in body_lower for term in SPAM_BLOCKLIST):
        return False

    return True


def format_issue_title(count: int) -> str:
    """Return correctly pluralized issue title without typos."""
    noun = "Opportunity" if count == 1 else "Opportunities"
    return f"🎯 Bounty Alert: {count} New {noun} found"


def format_notification_message(bounties: list, now_str: str) -> str:
    """Return formatted markdown message for Telegram / Discord notifications."""
    count = len(bounties)
    noun = "opportunity" if count == 1 else "opportunities"
    notif_lines = [
        f"🎯 *New Bounty Alert* ({now_str})",
        f"Found {count} new {noun}:\n",
    ]
...
def format_issue_body(bounties: list, now_str: str) -> str:
    """Return formatted markdown body for native GitHub issue notification."""
    issue_body = (
