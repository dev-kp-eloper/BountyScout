...
def is_clean_candidate(item, current_repo=None):
    ...
    if "active bounty scan results" in body_lower:
        return False

    if "/bountyscout/" in url_lower:
        return False
...
def pluralize_opportunity(count, capitalize=False):
    """Return the correct singular or plural form of the word opportunity.
    ...
    Returns:
        str: Formatted singular or plural opportunity string.
    """
    if count == 1:
...
def format_issue_title(count):
    """Format the GitHub issue title with correct singular or plural grammar.
    ...
    """
    word = pluralize_opportunity(count, capitalize=True)
...
def format_notification_message(bounties, now_str):
    ...
    """
    count = len(bounties)
    word = pluralize_opportunity(count, capitalize=False)
    lines = [
...
def build_notification_markdown(bounties, now_str):
    ...
    Returns:
        str: Notification message string.
    """
    return format_notification_message(bounties, now_str)


def build_github_issue_payload(bounties, now_str):
    """Build the title and body tuple for GitHub issue creation.
    ...
    """
    title = format_issue_title(len(bounties))
    body = format_issue_body(bounties, now_str)
    return title, body


def format_github_issue_content(bounties, now_str):
    """Alias for build_github_issue_payload.
    ...
    Returns:
        tuple: (issue_title, issue_body)
    """
    return build_github_issue_payload(bounties, now_str)
