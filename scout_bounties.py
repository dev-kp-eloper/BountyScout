...
def main():
    ...
    notif_lines = [
        ...
        f"Found {len(new_bounties)} new {'opportunity' if len(new_bounties) == 1 else 'opportunities'}:\n"
    ]
    ...
    if github_token and repo_fullname:
        issue_title = f"🎯 Bounty Alert: {len(new_bounties)} New {'Opportunity' if len(new_bounties) == 1 else 'Opportunities'} found"
        issue_body = (
