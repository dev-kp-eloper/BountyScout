...
def is_clean_candidate(item, current_repo=None):
    ...
    # 5. Skip self-referential / recursive alert issues from BountyScout
    if "bounty alert" in title_lower:
        return False
    if "/bountyscout/issues/" in html_url.lower():
        return False
    if current_repo and f"github.com/{current_repo.lower()}/issues/" in html_url.lower():
        return False
...
def format_issue_title(count):
    """Format the GitHub issue title with correct pluralization."""
...
def format_notification_message(bounties, now_str):
    """Format Telegram/Discord notification text with correct pluralization."""
    count = len(bounties)
    noun = "opportunity" if count == 1 else "opportunities"
    notif_lines = [
...
def format_issue_body(bounties, now_str):
    """Format the GitHub issue body in markdown."""
    issue_body = (
